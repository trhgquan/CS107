...
void Machine::IncreasePC() {
	...
	int tempPCReg = this->ReadRegister(PCReg);
	this->WriteRegister(PrevPCReg, tempPCReg);

	// Set the next PC resgister as current PC register
	tempPCReg = this->ReadRegister(NextPCReg);
	this->WriteRegister(PCReg, tempPCReg);

	// Set the current PC register + 4 as the next PC register
	this->WriteRegister(NextPCReg, tempPCReg + 4);
}
