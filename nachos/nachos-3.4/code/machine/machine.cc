...
/*
Purpose:	- Set the current PC register as previous PC register
- Set the next PC resgister as current PC register
- Set the current PC register + 4 as the next PC register
*/
void Machine::IncreasePC() {

	// set the current PC register as previous PC register
	int tempPCReg = machine->ReadRegister(PCReg);
	machine->WriteRegister(PrevPCReg, tempPCReg);

	// Set the next PC resgister as current PC register
	tempPCReg = machine->ReadRegister(NextPCReg);
	machine->WriteRegister(PCReg, tempPCReg);

	// Set the current PC register + 4 as the next PC register
	machine->WriteRegister(NextPCReg, tempPCReg + 4);
}
